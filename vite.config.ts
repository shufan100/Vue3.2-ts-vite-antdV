...
export default defineConfig(({ command, mode }: ConfigEnv): UserConfig => {
  ...
  const { VITE_PORT, VITE_PUBLIC_BASE, VITE_DROP_CONSOLE } = viteEnv
  ...
  return {
    ...
    base: VITE_PUBLIC_BASE, //  开发或生产环境服务的公共基础路径：默认'/'   1、绝对 URL 路径名： /foo/；  2、完整的 URL： https://foo.com/； 3、空字符串或 ./（用于开发环境）
    ...
    server: {
      ...
      host: true, // 监听所有地址
      port: VITE_PORT, //指定开发服务器端口：默认3000
    ...
    build: {
      ...
      cssCodeSplit: false, // false:将所有的 CSS 抽取到一个文件中，true:将一个异步 chunk 模块中使用到的 CSS 代码抽取为其生成一个单独的文件。  默认true
